...
export function StockLedger({ transactions, isLoading, unit }: StockLedgerProps) {
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-12">
        ...
        <span className="ml-3 text-sm text-gray-500 dark:text-gray-400">Loading ledger history...</span>
      </div>
    );
  }
  ...
  if (transactions.length === 0) {
    return (
      <div className="text-center py-12 px-4 bg-gray-50 dark:bg-black rounded-xl border border-dashed border-gray-200 dark:border-neutral-800">
        <History className="w-10 h-10 text-gray-300 mx-auto mb-3" />
        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">No transaction history</p>
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Stock IN/OUT records will appear here.</p>
      </div>
    );
  }
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-neutral-800 text-gray-600 dark:text-gray-400 font-medium">
